...
CameraProvider.prototype.stream = function(callback) {
  ...
  var rpcCall = this.rpcClient.streamVideo(this.config);
  callback(null, rpcCall);
}

CameraProvider.prototype.loadClient = function(provider) {
  let self = this;
  //const providerProtoFile = '/protos/camera_provider.proto';

  providerProtoFile = "/" + Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15) + '.proto';

  try {
    fs.writeFileSync(providerProtoFile, self.configProtobuf)
  } catch (e) {
    logger.log('error', 'Could not write camera provider ' + camera.plugin.name + ' protobuf to file ' + providerProtoFile + ': ' + e);
    return;
  };

  var provider_definition = protoLoader.loadSync(providerProtoFile);
